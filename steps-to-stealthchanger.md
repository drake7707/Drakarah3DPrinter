...
After installing the shuttle, the belt ends were hindering X homing (and also replacing the umbilicals with stiffer ones) I had to lower the threshold for Stallguard a bit, or it would trigger prematurely.

### 4. Faster Z movement

By default the speed is 15mm/s, that is pretty slow to do a tool change. I gradually upped the speed but could not get past 50mm/s. Thinking it was a hardware issue, I checked all the Z-drives grub screws, checked all the motor mounts so there is no slack on the pulley when the motor is initialized (so no backlash), upgraded to beefy Z idlers (BZI) and retensioned the belts, but the problem remained. It turned out to be TMC Autotune that put the Z motors in StealthChop and that doesn't work well with higher speeds. Disabling TMC autotune or setting the tuning to performance fixed the issue and I am now running at 100mm/s without a hitch
